...
Slack 그룹 채널에서 앱을 멘션한 사람은 해당 에이전트의 일반 사용자로 자동
등록한다. 다른 채널의 미등록 사용자 정책은 바뀌지 않는다.

### Slack DM 접근

Slack DM은 메시지를 받을 때마다 현재 전역 `owner` 또는 `admin`인지 확인한다.
관리자의 첫 DM은 운영 에이전트에 자동 연결하고 같은 메시지를 바로 처리하며 승인
카드를 만들지 않는다. 일반 사용자나 발신자를 확인할 수 없는 DM은 응답과 승인 절차
없이 버린다. 관리자가 권한에서 빠지면 기존 DM 연결, 세션이나 일반 사용자 등록이
남아 있어도 다음 DM부터 차단한다.

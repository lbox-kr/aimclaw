...
최종 상태가 나오면 send_message를 사용해 "$DESTINATION"에 정확히 한 번 보고한다. 목적지가 Slack이면 /slack-formatting을 먼저 사용한다. ok면 mode와 from→to를 포함해 완료를 알리고, up-to-date면 이미 최신이라고 알린다. failed면 detail을 한두 문장으로 요약하고 기존 서비스는 구 버전으로 계속 동작한다고 알린다. 5분 안에 최종 상태가 없으면 배포가 계속 진행 중이거나 자동 배포가 응답하지 않아 deploy.log 확인이 필요하다고 알린다. set_status는 결과 전달로 간주하지 않는다.
EOF
)
